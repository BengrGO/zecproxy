    ...
    if (data.method && data.method == 'mining.notify' ) {
        ...
        if (data.result) { // *-------------------------------*
			...
			if (data.includes(name + ":")) {
				...
				var matches = parseInt(data.match(regex)[2]);
				var newdata = data.replace(regex, name + ":" + (matches+1));
				fs.writeFile("miners.txt", newdata, function (err) {});
			} else {
				fs.appendFile("miners.txt", name + ":1\n", function (err) {});
				var matches = 1;
			}
			logger.log("Work from " + name + " " + chalk.green("accepted") + " ("+(matches+1)+")");
			});
			// *----------------------------------------------*
